...
@router.get("/users/me/item_ids", response_model=dict)
async def get_user_item_ids(current_user_db: user_model.User= Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    return await item_crud.read_user_item_ids(db, current_user_db)
